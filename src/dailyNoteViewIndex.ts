...
export default class DailyNoteViewPlugin extends Plugin {
    ...
    async onload() {
        ...
        this.addRibbonIcon(
            "calendar-range",
            "Open Simple Daily Notes",
            (evt: MouseEvent) => this.openDailyNoteEditor()
        );
        this.addCommand({
            id: "open-simple-daily-notes",
            name: "Open Simple Daily Notes",
            callback: () => this.openDailyNoteEditor(),
        });

        this.initCssRules();

        // Create daily note and open the Simple Daily Notes on startup if enabled
        if (this.settings.createAndOpenOnStartup) {
            this.app.workspace.onLayoutReady(async () => {
                ...
                if (
                    ...
                    return;
                // Then open the Simple Daily Notes
