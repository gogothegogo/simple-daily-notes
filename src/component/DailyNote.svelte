    ...
    onDestroy(() => {
        if (createdLeaf?.detach) {
            try {
                ...
                console.error("Error detaching leaf:", error);
            }
        }
    });
...
<div class="daily-note-container" data-id='dn-editor-{file.path}'>
    <div class="daily-note" data-collapsed={isCollapsed}>
        {#if title}
        ...
        {/if}
        <div class="daily-note-editor" data-collapsed={isCollapsed} aria-hidden="true" bind:this={editorEl} data-title={title} on:click={handleEditorClick}></div>
    </div>
</div>
